...
/**
 * Node model for the nodes converting between CWL ports.
 * 
 * @author Martin Horn, Alexander Fillbrunn: University of Konstanz
 */
public class AnyToSpecificNodeModel extends NodeModel {
     ...
     */
    public AnyToSpecificNodeModel(final CWLType type) {
    ...
    protected PortObject[] execute(final PortObject[] inData, final ExecutionContext exec) throws Exception {
        CWLPortObject po = (CWLPortObject)inData[0];
        PortObject output;
        try {
            output = CWLPortObject.createOutput(m_type, po.getJsonContent());
        } catch (Exception e) {
            throw new InvalidSettingsException(String.format("Cannot convert from %s to %s.",
                    po.getType().symbol(), m_type.symbol()), e);
        }
