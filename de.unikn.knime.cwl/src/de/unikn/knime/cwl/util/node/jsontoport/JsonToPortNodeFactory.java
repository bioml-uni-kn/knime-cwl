...
/**
 * Factory for the nodes converting JSON to CWL ports.
 *
 * @author Martin Horn, Alexander Fillbrunn: University of Konstanz
 */
public class JsonToPortNodeFactory 
        extends NodeFactory<JsonToPortNodeModel> {
    
    private static final NodeLogger LOGGER = NodeLogger.getLogger(JsonToPortNodeFactory.class);
     ...
     */
    @Override
    public JsonToPortNodeModel createNodeModel() {
        return new JsonToPortNodeModel(m_type);
    }
     ...
     */
    @Override
    public NodeView<JsonToPortNodeModel> createNodeView(final int viewIndex,
            final JsonToPortNodeModel nodeModel) {
        return null;
    }
    ...
    protected NodeDescription createNodeDescription() throws SAXException, IOException, XmlException {
        try {
            ...
            node.setName(String.format("JSON to CWL %s", niceType));
            
            node.setIcon("./jsontocwl.png");
            node.setType(KnimeNode.Type.OTHER);
            
            String descr = String.format("Converts a JSON cell into a CWL Port of type %s.", niceType);
            ...
            ip.setName("JSON Input");
            ip.getDomNode().appendChild(domDoc.createTextNode("Table with a JSON column"));
